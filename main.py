...
if __name__ == '__main__':
    ...
    parser.add_argument("-p", "--problem", help = "Which problem?", required = False, default = "1")
    parser.add_argument("-d", "--day", help = "Which day?", required = False, default = day)
    parser.add_argument("-y", "--year", help = "Which year?", required = False, default = year)
    ...
    msg = "Running script for Advent of Code {} on day {} for problem {}".format(arg.year, arg.day, arg.problem)
    logger.log(msg, 'info')

    if int(arg.problem) == 1:
        test_solution(1)
        print('Problem 1 solution:', sol.p1(path_to_day+'in.txt'))
    elif int(arg.problem) == 2:
        test_solution(2)
        print('Problem 2 solution:', sol.p2(path_to_day+'in.txt'))
    else:
        raise AssertionError(
            'Incorrect argument passed. Please send 1 or 2 for the problem you are trying to solve.')
